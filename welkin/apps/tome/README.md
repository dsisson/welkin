...
I created this pull request because I wanted to get a feel for Tome's code, and to demonstrate my approach to test automation. 

Granted, I wrote this against the Tome's marketing site, which is completely different from what your product *does*. I mean, wow, that is going to be hard to test.

On the other hand, the value of this approach is it already has a way to run tests, so all the attention and effort can go to figuring out how to model and test the actual product.   

This PR includes:
* a wrapper for the Tome marketing site. This wrapper uses an advanced page object model built on my router design pattern.
