...
This pull request is a working proof-of-concept of an advanced end-to-end test automation framework for the AffiniPay marketing site. This framework demonstrates the role of advanced modeling in a modern automation approach. Granted, this is a marketing site, and so is not reflective of core product engineering work, but the value of this approach is that this POC already can run very advanced tests, so it's already an effective learning tool, ready to run.   

Included:
* a wrapper for the marketing site. This wrapper uses an advanced page object model built on my router design pattern, which allows better modeling of user site behavior.
* configuration changes to support the wrapper as a test fixture.
* a simple linear navigation test that moves around the limited set of pages I've modeled. (Note that I only supported pages that we served from https://www.affinipay.com/: the product and developer pages come from a different servers, which is out of scope for this POC.) 
...
I'm happy to provide test artefacts if you're interested. 
